...
1. 首頁的 **Start** 是引導式流程：Designer 上傳 PDF、建立或編排本專案欄位。
2. 在設計階段選擇「下一步」時，系統在瀏覽器記憶體中產生仍可編輯的 AcroForm 範本，並自動帶入 Signer；不需要下載後再上傳，也不會建立 JSON sidecar。
3. 返回設計階段會還原該次流程的設計快照；「重新開始」或重新整理頁面會清除這個暫存快照。若只需單一工作，可用弱化的「只設計」與「直接簽名」快速入口。
4. Signer 填寫支援的 AcroForm 欄位，匯出仍可編輯的 PDF。只有使用者明確選擇「扁平化定稿」時，才將欄位內容轉為不可編輯的頁面內容。
